...
that project."
  ...
  (let ((time-string (timeclock-seconds-to-string (timeclock-last-period)))
	...
	(project-string (if (timeclock-currently-in-p)
						(nth 2 timeclock-last-event)))
	(bang (if timeclock-query-project-timer "!")))

	(if (> (length project-string) 20)
		(setq project-string (substring project-string 0 20)))

    (setq timeclock-mode-string
	  (if (timeclock-currently-in-p)
	      (concat "[" job-string project-string " " time-string "]" bang)
	    (concat "<" job-string
