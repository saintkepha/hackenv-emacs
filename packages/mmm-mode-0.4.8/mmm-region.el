...
region of that mode is present at POS, or nil if none."
  ...
  (setq mmm-current-submode mode
        mmm-current-overlay (find-if #'(lambda (ovl)
...
is non-nil, don't quit if the info is already there."
  (message "updating mode info for %s" mode)
    ...
    (unless (and (not force)
      ...
      (save-excursion
        ...
        (when (featurep 'font-lock)
		  ...
		  (put mode 'mmm-fontify-buffer-function
...
(defun mmm-refontify-maybe (&optional start stop)
  ...
  (message "mmm-refontify-maybe called!")
  (and font-lock-mode
       (if (or start stop)
...
(defun mmm-fontify-buffer ()
  "Fontify (or no-fontify) the entire buffer"
  (message "Not Fontifying the buffer..."))

(defun mmm-fontify-region (start stop &optional loudly)
  "Fontify from START to STOP keeping track of submodes correctly."
  (when (or loudly t)
    (message "Fontifying %s from %d to %d..." (buffer-name) start stop))
...
;	  (message "fontify %d buffer for %s" (length regions) mode)
	   (mapc #'(lambda (reg)
				 (goto-char (car reg))
				 ...
				 (mmm-set-local-variables mode)
				 (message "Fontifying %s with %d regions" mode (length regions))
